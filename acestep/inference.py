...
def _estimate_calculated_vocal_syllable_capacity(
    ...
    estimated_syllables = int(round(vocal_beats * _LYRIC_DENSITY_SYLLABLES_PER_BEAT * meter_factor))
...
def _normalize_lyrics_for_generation(lyrics: str) -> str:
    ...
    for line in lyrics.splitlines():
        ...
        pieces = [piece.strip() for piece in _LYRIC_COMMA_LINEBREAK_PATTERN.split(stripped) if piece.strip()]
        normalized_lines.extend(pieces or [stripped])
