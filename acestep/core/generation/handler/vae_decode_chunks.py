...
class VaeDecodeChunksMixin:
    ...
    def _tiled_decode_inner(
        ...
        overlap,
        offload_wav_to_cpu,
        decode_progress_callback: Optional[Callable[[int, int], None]] = None,
    ):
        ...
        if bsz > 1:
            ...
            for b_idx in range(bsz):
                ...
                if callable(decode_progress_callback):
                    decode_progress_callback(b_idx + 1, bsz)
                self._empty_cache()
    ...
    def _tiled_decode_gpu(
        ...
        for i in tqdm(range(num_steps), desc="Decoding audio chunks", disable=self.disable_tqdm):
            ...
            if callable(decode_progress_callback):
                decode_progress_callback(i + 1, num_steps)
    ...
    def _tiled_decode_offload_cpu(
        ...
        if callable(decode_progress_callback):
            decode_progress_callback(1, num_steps)
