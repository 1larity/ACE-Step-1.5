...
class ServiceGenerateExecuteMixinTests(unittest.TestCase):
    ...
    def test_build_generate_kwargs_adds_timesteps_tensor(self):
        ...
        self.assertFalse(kwargs["disable_tqdm"])

    def test_build_generate_kwargs_disables_progress_bar_when_tqdm_disabled(self):
        """When host disables tqdm, kwargs should disable diffusion progress bar."""
        host = _Host()
        host.disable_tqdm = True
        payload = {
            "text_hidden_states": torch.zeros(1, 2),
            "text_attention_mask": torch.ones(1, 2),
            "lyric_hidden_states": torch.zeros(1, 2),
        ...
        self.assertFalse(kwargs["use_progress_bar"])
        self.assertTrue(kwargs["disable_tqdm"])

    def test_build_generate_kwargs_forwards_diffusion_progress_callback(self):
        """Runtime callback should be wrapped and passed to model generation kwargs."""
        host = _Host()
        events = []
        ...
        kwargs["progress_callback"](2, 8, "DiT diffusion steps")
        self.assertEqual(events[0]["stage"], "diffusion")
        self.assertEqual(events[0]["current"], 2)
        self.assertEqual(events[0]["total"], 8)
