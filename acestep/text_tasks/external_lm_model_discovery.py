"""Provider-aware external model discovery helpers for UI model auto-population."""
...
class ExternalModelDiscoveryError(RuntimeError):
    """Raised when model discovery requests fail across all candidate endpoints."""
...
def discover_external_models(
    *,
    ...
    """Discover available model identifiers for the selected external provider."""
    ...
    if not candidate_urls:
        raise ExternalModelDiscoveryError("No model-discovery endpoint could be derived from base URL.")
...
def _build_model_list_urls(*, provider: str, protocol: str, base_url: str) -> list[str]:
    """Build ordered candidate model-list endpoints from provider/protocol/base URL."""
    ...
    if protocol_token == "anthropic_messages" and "/models" not in root:
        if "/v1/" in root:
...
def _build_auth_headers(*, protocol: str, api_key: str) -> dict[str, str]:
    """Build protocol-compatible auth headers for model-discovery requests."""
    headers = {"Content-Type": "application/json"}
    token = (protocol or "").strip().lower()
    if token == "anthropic_messages":
        if api_key:
...
def _extract_model_ids(payload: Any) -> list[str]:
    """Extract model IDs from common provider response shapes."""
...
def _add_model_id(collector: OrderedDict[str, None], item: Any) -> None:
    """Add normalized model identifier from a response item into collector."""
