...
class InitServiceWrapperDeviceResolutionTests(unittest.TestCase):
    ...
    @patch("acestep.ui.gradio.events.generation.service_init.get_global_gpu_config")
    def test_external_lm_selection_skips_local_init_and_enables_think(self, mock_gpu_config):
        """An external LM dropdown entry should not trigger local LM initialization."""
        ...
        llm_handler.initialize.assert_not_called()
        self.assertIn("External LM provider active", result[0])
        self.assertTrue(result[-1]["interactive"])
        self.assertTrue(result[-1]["value"])

    @patch("acestep.ui.gradio.events.generation.service_init.warm_up_external_provider")
    @patch("acestep.ui.gradio.events.generation.service_init.get_global_gpu_config")
    def test_ollama_external_selection_runs_warmup(self, mock_gpu_config, warmup_mock):
        """Ollama external selections should trigger a best-effort warm-up during init."""
        ...
        warmup_mock.return_value = "Ollama warm-up complete (qwen3:8b)"
        ...
        self.assertIn("Ollama warm-up complete", result[0])
